...
function buildWhatsappSummary(p: PropertyDetail): string {
  const emoji = propertyTypeEmoji[p.propertyType] || "🏠";
...
export default function PropiedadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const whatsappSummary = buildWhatsappSummary(property);
  ...
  function shareWhatsapp() {
    const url = `https://wa.me/?text=${encodeURIComponent(whatsappSummary)}`;
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl shadow-sm p-6 border border-green-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button onClick={copyWhatsapp} className="bg-white text-gray-700 hover:bg-gray-50 border border-gray-200 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                Copiar
              </button>
              <button onClick={shareWhatsapp} className="bg-green-600 text-white hover:bg-green-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg>
                Enviar
              </button>
            </div>
          </div>
          <pre className="bg-white rounded-lg p-4 text-sm text-gray-800 font-sans whitespace-pre-wrap border border-gray-100 leading-relaxed">{whatsappSummary}</pre>
          <p className="text-xs text-gray-500 mt-2">Se genera automáticamente con los datos de la propiedad. Agrega extras en el campo &ldquo;Notas adicionales WhatsApp&rdquo; al editar.</p>
        </div>
