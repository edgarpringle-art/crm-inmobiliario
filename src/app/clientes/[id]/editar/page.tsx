...
function parseZones(raw: string | string[] | null | undefined): string[] {
...
export default function EditarClientePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/clients/${id}`)
      .then((r) => r.json())
      .then((data) => {
        // Prefer searchZones; fallback to legacy searchZone
        let zones = parseZones(data.searchZones);
        if (zones.length === 0 && data.searchZone) zones = [data.searchZone];
        ...
        setForm({
          ...
          bathroomsMax: data.bathroomsMax?.toString() || "",
          ...
          hasAppliances: !!data.hasAppliances, hasAC: !!data.hasAC,
          hasBalcony: !!data.hasBalcony, hasGarden: !!data.hasGarden,
        ...
        {needsSearch && (
          <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-blue-400">
            ...
            <FormField label="Notas de búsqueda">
              <textarea className={inputClass} rows={3} value={form.searchNotes} onChange={(e) => update("searchNotes", e.target.value)} placeholder="Cualquier detalle adicional..." />
            </FormField>
          </div>
        )}
